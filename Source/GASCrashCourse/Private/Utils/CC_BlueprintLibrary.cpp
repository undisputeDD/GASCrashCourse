...
FClosestActorWithTagResult UCC_BlueprintLibrary::FindClosestActorWithTag(UObject* WorldContextObject, const FVector& Origin, const FName& Tag, float SearchRange)
{
	...
	for (AActor* Actor : ActorsWithTag)
	{
		...
		ACC_EnemyCharacter* EnemyCharacter = Cast<ACC_EnemyCharacter>(WorldContextObject);
		if (!IsValid(EnemyCharacter) || Distance > EnemyCharacter->SearchRange) continue;
...
void UCC_BlueprintLibrary::SendDamageEventToPlayers(TArray<AActor*> Targets, const TSubclassOf<UGameplayEffect>& DamageEffect, UPARAM(ref)FGameplayEventData& Payload, const FGameplayTag& DataTag, float Damage, const FGameplayTag& EventTagOverride, UObject* OptionalParticleSystem)
{
...
TArray<AActor*> UCC_BlueprintLibrary::HitBoxOverlapTest(AActor* AvatarActor, float HitBoxRadius, float HitBoxForwardOffset, float HitBoxElevationOffset, bool bDrawDebugs)
{
	...
	if (bDrawDebugs)
	{
		DrawHitBoxOverlapResults(World, OverlapResults, HitBoxLocation, HitBoxRadius);
	}
...
TArray<AActor*> UCC_BlueprintLibrary::ApplyKnockback(AActor* AvatarActor, const TArray<AActor*>& HitActors, float InnerRadius, float OuterRadius, float LaunchForceMagnitude, float RotationAngle, bool bDrawDebugs)
{
	...
	{
		ACC_EnemyCharacter* HitCharacter = Cast<ACC_EnemyCharacter>(HitActor);
		...
		HitCharacter->StopMovementUntilLanded();
	}
...
{
	DrawDebugSphere(World, HitBoxLocation, HitBoxRadius, 16, FColor::Blue, false, 3.f);
	...
	for (const FOverlapResult& Result : OverlapResults)
	{
		if (IsValid(Result.GetActor()))
		{
			...
			DrawDebugSphere(World, DebugLocation, 30.f, 10, FColor::Black, false, 3.f);
		}
	}
}